...
| FR-OFFLINE-07 | Die Outbox arbeitet ihre Einträge **abhängigkeitsgerecht** ab: Der Inhalt einer offline angelegten Notiz - und mit ihm der Upload ihrer lokalen Bildentwürfe - wird erst übertragen, wenn die Seite serverseitig existiert. Steht das Anlegen noch aus, bleibt der Eintrag offen; ist es dauerhaft gescheitert, nennt die Meldung des Inhaltseintrags **diese** Ursache statt eines Folgefehlers am Bild. Hat eine bereits angelegte Seite ihre Server-Kennung, wird ein auf der Temporär-Kennung stehengebliebener Eintrag nachgezogen. Ein **endgültiges** Hindernis - etwa ein lokal nicht mehr vorhandener Bildentwurf - blockiert ausschließlich den betroffenen Eintrag und wird gemeldet; die übrigen Einträge laufen weiter. Nur vorübergehende Fehler (Netz, Rate Limit) führen zu einer geplanten Wiederholung. | M |
...
| FR-NOTE-28 | Im Kopf einer geöffneten **Notiz** steht rechts neben „In den Papierkorb" ein **Kopie**-Schalter (`POST /api/pages/{id}/duplicate`). Die Kopie entsteht serverseitig samt eingebetteten Bildern und Dateianhängen (`PageCopyService`, dieselbe Mechanik wie FR-SHR-16), heißt **„Kopie von <Titel>"** — auf 200 Zeichen gekürzt wie jeder Titel — und ist von der Vorlage unabhängig; Versionen, Freigaben und der Favoritenstatus werden nicht mitkopiert. Bei einer **eigenen** Notiz landet sie ohne Rückfrage im **selben Notizbuch**. Ein Nutzer, dem die Notiz **geteilt** wurde (Seiten- oder Notizbuchfreigabe, lesend wie schreibend), kann sie jederzeit ebenso kopieren — die Kopie entsteht dann in **seinem** Workspace, und weil das Notizbuch der Vorlage dem Eigentümer gehört und Empfängern verborgen bleibt, fragt ein Dialog nach dem Zielnotizbuch (Vorgabe: ohne Notizbuch). Nur eigene Notizbücher stehen zur Wahl. Die Kopie zählt auf das Speicherkontingent des kopierenden Nutzers (FR-ADM-06), ist auf 30 Kopien pro Stunde und Konto begrenzt und wird im Audit-Log vermerkt. Aufgabenlisten und Logbücher haben den Schalter **nicht**; verschlüsselte Notizen können serverseitig nicht kopiert werden (FR-CRYPT-05). | S |
...
| FR-SHR-08 | Mit Schreibfreigabe kann ein angemeldeter Nutzer Inhalte ändern und Tasks importieren, aber **nicht**: Seite löschen, Freigaben verwalten, Seitentyp ändern, Seite im Workspace des Eigentümers duplizieren, Versionsverlauf wiederherstellen. Eine Kopierfreigabe erzeugt eine unabhängige Seite samt Bildern und Dateianhängen im Workspace des Empfängers; eine geteilte **Notiz** kann der Empfänger davon unabhängig jederzeit in seinen eigenen Workspace kopieren (FR-NOTE-28). | M |
...
| FR-LOG-12 | Ein Logbuch lässt sich über einen **Export**-Schalter im Seitenkopf herunterladen; ein Aufklappmenü bietet **CSV**, **Excel (XLSX)** und **XML**. Der Export enthält die Kopfzeile (Zeitpunkt + Spaltennamen) und alle Einträge, aufsteigend nach Zeitpunkt. Zahlenspalten gehen als Zahlen heraus (Punkt als Dezimaltrennzeichen, in XLSX als Zahlenzelle), Bewertungen als Zahl von 0 bis 5 statt als Sterne, damit sich damit rechnen lässt; die Benutzerspalte trägt den Anleger des Eintrags. Zeitpunkte liegen als UTC in der Datenbank und werden in die Zeitzone des Browsers umgerechnet, die der Aufruf mitschickt (unbekannte Zone: UTC). CSV trägt ein BOM, damit Excel die Umlaute erkennt; XLSX wird ohne weitere Abhängigkeit erzeugt und enthält den Zeitpunkt als echtes Datum. Lesender Zugriff genügt. | S |
...
| FR-VOICE-04 | Wird die Aufnahme aus einem **geöffneten Notizbuch** heraus gestartet, entsteht die Sprachnotiz in genau diesem Notizbuch — die gewählte Sammlung hat Vorrang. Nur ohne ausgewähltes Notizbuch (Startseite, Favoriten, Geteilt, Papierkorb) wird das **passende Notizbuch aus dem Inhalt** abgeleitet: Das Modell wählt aus den vorhandenen Notizbüchern des Workspaces. Zugeordnet wird nur bei Namensgleichheit (ohne Rücksicht auf Groß-/Kleinschreibung); ein erfundener Name legt **kein** neues Notizbuch an, die Notiz bleibt dann unzugeordnet. | S |
...
| FR-VOICE-07 | Störungen des Anbieters (nicht erreichbar, Schlüssel abgelehnt, unlesbare Antwort) liefern HTTP 502 mit dem Code `VOICE_SERVICE_FAILED` statt eines Serverfehlers; die Meldung enthält nie den Schlüssel. Das Anlegen der Notiz erfolgt erst nach erfolgreicher Transkription — eine gescheiterte Aufnahme hinterlässt keine leere Seite. Liefert allein die **Nachbearbeitung** nichts Verwertbares, entsteht die Notiz aus dem Rohtranskript; die bereits gelöschte Aufnahme geht nicht verloren. | M |
| FR-VOICE-08 | Vor jeder Aufnahme für eine Notiz wählt der Nutzer eine **Vorlage**: eine benannte Anweisung an das Modell, wie das Diktat aufzubereiten ist (etwa als Angebot mit Position, Menge und Preis), optional mit eigenem **Vokabular** für Fachbegriffe. Die Auswahl ist Pflicht — beide Diktatwege einer Notiz (neue Notiz, Diktat in die offene Notiz) verlangen ein gültiges `template_id`. Logbuch- und Aufgaben-Diktat sowie NotesVoice bleiben ohne Vorlage. | S |
| FR-VOICE-09 | Vorlagen sind entweder **global** (vom Admin gepflegt, für alle wählbar) oder **persönlich** (vom Nutzer selbst unter „Speech2Text › Vorlagen" gepflegt). Sichtbar und verwendbar sind ausschließlich globale und eigene Vorlagen; eine fremde Vorlagen-ID wird abgewiesen. Je Bereich sind 20 Vorlagen zulässig, der Name höchstens 80, die Anweisung höchstens 8000 und das Vokabular höchstens 600 Zeichen. Die **letzte globale Vorlage** lässt sich nicht löschen, damit das Diktat nie ohne wählbare Vorlage dasteht. | M |
| FR-VOICE-12 | Eine per Diktat entstandene Notiz **merkt sich ihre Vorlage**. Ein weiteres Diktat in dieselbe Notiz fragt nicht erneut, sondern nimmt sofort auf und verwendet sie wieder. Das Modell bekommt dabei den bisherigen Notizinhalt als Vorbild (höchstens 12000 Zeichen) und die Anweisung, die Notiz in genau dieser Form fortzuführen — gleiche Gliederung, fortlaufende Positionsnummern — und ausschließlich den neuen Teil zurückzugeben, der unten angefügt wird. Auch das erste Diktat in eine bisher handgetippte Notiz hinterlegt die dabei gewählte Vorlage. Löscht der Admin die Vorlage, fragt die Notiz wieder nach; ihr Inhalt bleibt unberührt. | S |
| FR-VOICE-11 | Globale Vorlagen kann jeder Nutzer **für sich abschalten**, damit die Auswahl vor der Aufnahme kurz bleibt. Abgeschaltete erscheinen nicht mehr in der Auswahl, bleiben in den Einstellungen aber sichtbar und jederzeit wieder einschaltbar. Die Abwahl gilt je Nutzer und lässt andere unberührt; neu angelegte globale Vorlagen sind für alle zunächst aktiv. Eigene Vorlagen kennen das nicht — wer seine nicht mehr braucht, löscht sie. In beiden Verwaltungen (Nutzer und Admin) zeigt die Liste nur Name und Zustand; Anweisung und Vokabular klappen auf Klick auf. | M |
| FR-VOICE-10 | Die Anweisung der Vorlage ergänzt die im Dashboard gepflegte Anweisung aus FR-VOICE-05, ersetzt sie nicht: Antwortformat, Markdown-Regeln und Notizbuchzuordnung bleiben zentral. Das Vokabular geht zusätzlich als Ausspracheshilfe an die Transkription, die davon die letzten 224 Tokens auswertet. Eine Migration legt die globale Vorlage „Standard" an, damit das Diktat unmittelbar nach der Aktualisierung weiter nutzbar ist. | M |
...
| NFR-UI-24 | Mobil (< 768 px) zeigt die Werkzeugleiste der Notizseite nur die beim Schreiben am Telefon gebräuchlichen Werkzeuge in **einer** Zeile: Fett, Kursiv, Überschrift 2, Aufzählung, Checkliste, Foto aufnehmen, Diktat, Rückgängig. Die übrigen (Durchgestrichen, Code, Codeblock, Überschrift 1, Zitat, Link, Tabelle, Bild, Anhang, KI, Wiederholen) klappt ein Mehr-Schalter am Ende der Zeile auf; der Zustand bleibt bis zum erneuten Umlegen bestehen. Die Tabellenknöpfe erscheinen unabhängig davon, sobald die Einfügemarke in einer Tabelle steht. Maßgeblich ist die Breite der **Werkzeugleiste**, nicht die des Fensters (CSS-Container-Query an `.editor-toolbar-collapsible`, nicht an `.editor-toolbar` - dieselbe Klasse trägt der Werkzeugkasten des Annotations-Dialogs, dessen Farbpalette der Stapelkontext einer Container-Query verdecken würde): Neben der Seitenliste bleiben schon ab 768 px Fensterbreite nur rund 360 px für die Spalte übrig. Unter 640 px Leistenbreite wird eingeklappt und der Mehr-Schalter erscheint; darüber steht die Leiste vollständig und der Schalter entfällt - er ist also **nur** sichtbar, solange tatsächlich Werkzeuge verborgen sind. | M |
| NFR-UI-25 | Mobil (< 768 px) bleibt der Kopf der Notizseite **einzeilig**: Zurück-Schalter und Aktionen stehen in derselben Zeile, der Hinweis „Geteilt“ samt Berechtigung und die Personenkreise der Mitbearbeiter entfallen dort. Dass eine Seite geteilt ist, zeigt stattdessen der eingefärbte Teilen-Schalter; sein Titel nennt den Zustand. Ab 768 px stehen Hinweis und Personenkreise wie bisher daneben. Die **Beschriftungen** der Aktionen hängen dagegen an der Breite des Seitenkopfs, nicht an der des Fensters (CSS-Container-Query an `.page-toolbar-note`): Unter 750 px steht je Aktion nur das Symbol, dessen `title`/`aria-label` die Bedeutung trägt. So bleibt der Kopf auch bei schmaler Inhaltsspalte - breite Notizbuchleiste, geöffnete Seitenliste - lesbar, statt die Schaltflächen zu stauchen. Reicht der Platz dennoch nicht, bricht die Leiste um. Die Köpfe von Logbuch und Aufgabenliste tragen höchstens drei Aktionen und behalten deshalb die einfache Fenster-Schwelle. | M |
...
| POST | `/api/pages/{id}/duplicate` | Notiz in den eigenen Workspace kopieren (optional `notebook_id`; ohne Angabe dasselbe Notizbuch wie die Vorlage, FR-NOTE-28) |
...
| **POST** | **`/api/voice/transcribe`** | **Aufnahme transkribieren, ohne zu speichern (multipart, Felder `audio`, `page_id`; `template_id` nur nötig, solange die Notiz keine Vorlage kennt); liefert Transkript, aufbereiteten Text als ProseMirror-Dokument, Überschriftsvorschlag, Notizbuch und die verwendete `template_id` (FR-VOICE-02/03/08/12)** |
| **POST** | **`/api/voice/notes`** | **Aus einer Aufnahme direkt eine Notiz anlegen (multipart, Felder `audio`, `template_id`, optional `notebook_id` der gewählten Sammlung sowie `lat`/`lon`/`accuracy`); liefert die neue Seite (FR-VOICE-01–04/08, FR-NOTE-25)** |
| **GET** | **`/api/voice/templates`** | **Wählbare Diktier-Vorlagen: eigene und die aktiven globalen, global zuerst (FR-VOICE-08/09/11)** |
| **GET/POST** | **`/api/profile/voice-templates`** | **Verwaltungsansicht: eigene Vorlagen und alle globalen mit ihrem Aktiv-Zustand; anlegen mit `name`, `instruction`, `vocabulary` (FR-VOICE-09/11)** |
| **PATCH** | **`/api/profile/voice-templates/{id}/active`** | **Globale Vorlage für sich ein-/ausblenden (`active`); eigene liefern 404 (FR-VOICE-11)** |
| **PATCH/DELETE** | **`/api/profile/voice-templates/{id}`** | **Eigene Vorlage ändern bzw. löschen; fremde und globale liefern 404 (FR-VOICE-09)** |
| **GET/POST** | **`/api/admin/voice-templates`** | **Globale Vorlagen auflisten bzw. anlegen (Admin, FR-VOICE-09)** |
| **PATCH/DELETE** | **`/api/admin/voice-templates/{id}`** | **Globale Vorlage ändern bzw. löschen (Admin); die letzte lässt sich nicht löschen (FR-VOICE-09)** |
