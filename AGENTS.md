...
- For requirements context consult `docs/URS.md`; concept docs `docs/KONZEPT-NOTE-ENCRYPTION.md` and `docs/KONZEPT-NOTESVOICE.md` cover page encryption and voice notes, `docs/API-DESKTOP-ASSISTANT.md` is the frozen client contract for the Desktop Assistant endpoints. Prefer `composer.json`, `package.json`, PHP config, and CI when they disagree with the specification.
