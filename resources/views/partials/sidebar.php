...
>
    ...
    <div class="space-y-3 p-3">
        <form x-show="activeCollection !== 'trash'" @submit.prevent="search" class="flex items-center gap-2 rounded-md border bg-white px-2 py-2" style="border-color: var(--color-border);"><span x-icon="search"></span><input x-model="searchQuery" @input="if (!searchQuery) searchResults = []" type="search" placeholder="Suchen und Enter drücken…" class="sidebar-search min-w-0 flex-1 bg-transparent outline-none"><button type="button" x-show="searchQuery" x-cloak @click="clearSearch" class="icon-action" aria-label="Suche zurücksetzen" x-icon="x"></button></form>
