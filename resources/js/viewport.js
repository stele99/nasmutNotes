...
// damit auf den sichtbaren Ausschnitt zusammen. Der Versatz selbst wird bewusst
// *nicht* ausgeglichen: Rückte die Anwendung um ihn nach unten, geriete die
// Einfügemarke wieder tiefer, Safari verschöbe erneut und der Ausgleich schaukelte
// sich auf - gemessen wurden so schon 200 px Leerraum über der Leiste. Endet die
// Anwendung über der Tastatur, hat Safari von vornherein keinen Grund mehr zu
// verschieben.
...
function createProbe(win) {
  ...
  return (inset) => {
    ...
    box.textContent = [
      ...
      `page ${Math.round(viewport.pageTop)}  scrollY ${Math.round(win.scrollY)}  inset ${inset}`,
